...
public class EmojiBuilder  {
	...
	private const string OutputPath = "Assets/Emoji/Output/";//生成图集的路径
	private const string InputPath = "/Emoji/Input/";//添加文件的路径
	...
	public static void BuildEmoji()
	{
		...
		Dictionary<string,int> sourceDic = new Dictionary<string,int> ();//新字典

		string[] files = Directory.GetFiles (Application.dataPath + InputPath,"*.png");//获取路径
		...
		foreach (string key in sourceDic.Keys)//名字
        {
			...
			for (int index = 0; index < sourceDic[key]; index++) {
				...
				Color[] colors = asset.GetPixels (0); //获取一块像素颜色

				for (int i = 0; i < EmojiSize; i++) {//大小
				...
				if (! emojiDic.ContainsKey (key)) {
					...
					{
						info.key =   "["+char.ToString(keylist[keyindex])+"]" ;//添加索引值
					}else
					{
